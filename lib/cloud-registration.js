...
/**
 * When local SQLite is empty (typical on Vercel cold starts without hydrated data),
 * ask Atomic Center whether this user already registered a device.
 */
async function resolveOnboardingFromCloud({ meshUserId, username }) {
  const cloud = await checkCloudRegistration({
    userId: meshUserId,
    username,
  ...
  if (cloud.registered) {
    deviceProfile.markUserOnboarded({ meshUserId, email: null });
    return { onboardingComplete: true, cloudChecked: true, fromCloud: true };
  }

  return { onboardingComplete: false, cloudChecked: true, fromCloud: true };
}

module.exports = {
  checkCloudRegistration,
  syncOnboardingWithCloud,
  resolveOnboardingFromCloud,
};
