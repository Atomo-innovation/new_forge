...
function ensureDemoEvents(slug, camera) {
  ...
  if (Array.isArray(state.recentEvents) && state.recentEvents.length >= 5) {
  ...
  const specs = isFace
    ? [
      { title: 'Known Face Match', severity: 'success', identity: 'known' },
      { title: 'Face Detected', severity: 'warning', identity: 'unknown' },
      { title: 'Unknown Face', severity: 'warning', identity: 'unknown' },
      { title: 'Face Detected', severity: 'warning', identity: 'unknown' },
      { title: 'Known Face Match', severity: 'success', identity: 'known' },
      { title: 'Face Detected', severity: 'warning', identity: 'unknown' },
    ]
    : [
      { title: 'Person Detected', severity: 'warning' },
      { title: 'Presence Detected', severity: 'success' },
      { title: 'Person Detected', severity: 'warning' },
      { title: 'Person Detected', severity: 'warning' },
      { title: 'Presence Detected', severity: 'success' },
      { title: 'Person Detected', severity: 'warning' },
  ...
  const events = specs.map((spec, i) => {
    ...
    const detection = {
      ...
      score: 0.78 + (i % 3) * 0.06,
      box: [0.12 + (i % 4) * 0.1, 0.2, 0.22 + (i % 4) * 0.1, 0.45],
    };
    if (isFace) detection.identity = spec.identity;
    const evt = makePersonEvent({
      title: spec.title,
      camera,
  ...
  saveModelState(slug, { recentEvents: events });
  return events;
}
