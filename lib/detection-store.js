...
const { sanitizeCameraForClient } = require('./demo-cameras');
...
function getPayload(slug) {
  ...
  return {
    tab,
    state,
    assignedCameras: assigned.map(sanitizeCameraForClient),
    availableCameras: unassigned.map(sanitizeCameraForClient),
