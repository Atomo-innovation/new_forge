...
function ensureDemoEvents(slug, camera) {
  ...
  const DEMO_EVENTS_VERSION = 2;
  const state = getModelState(slug);
  const existing = Array.isArray(state.recentEvents) ? state.recentEvents : [];
  const imagesMatch = existing.length >= DEMO_EVENT_IMAGES.length
    && DEMO_EVENT_IMAGES.every((url, i) => existing[i]?.imageUrl === url);

  if (state._demoEventsVersion === DEMO_EVENTS_VERSION && imagesMatch) {
    return existing;
  }
