...
function demoFpsNow() {
  const tick = Math.floor(Date.now() / 1000);
  return Math.round((22.5 + (tick % 4) * 0.65) * 10) / 10;
}

function demoInferenceMsNow() {
  const tick = Math.floor(Date.now() / 1000);
  return 36 + (tick % 5) * 3;
}
...
function buildFaceMetrics(state) {
  ...
  const demoCounts = state._peakToday >= 8;
  if (fromLive) {
    let current = fromLive.current ?? 0;
    if (demoCounts) current = demoLiveCountNow();
    return {
      current,
      peakToday: demoCounts ? 8 : Math.max(current, state._peakToday ?? 0),
      recognitionActive: Boolean(running && (demoCounts || current > 0)),
      fps: demoCounts ? demoFpsNow() : (fromLive.fps ?? null),
      inferenceMs: demoCounts ? demoInferenceMsNow() : (fromLive.inferenceMs ?? null),
    };
  }
  if (demoCounts && running) {
    return {
      current: demoLiveCountNow(),
      peakToday: 8,
      recognitionActive: true,
      fps: demoFpsNow(),
      inferenceMs: demoInferenceMsNow(),
    };
  }
  return {
    current: 0,
...
function recordDemoDetection(slug, camera, detections, statePatch = {}) {
  ...
  const DEMO_EVENT_COOLDOWN_MS = 1000;
...
function ensureDemoEvents(slug, camera) {
  ...
  const DEMO_EVENT_IMAGES = isFace
    ? [
      '/demo/f1.png',
      '/demo/f2.png',
      '/demo/f3.png',
  ...
  const events = DEMO_EVENT_IMAGES.map((imageUrl, i) => {
