...
const { isDemoCamera, getDemoVideoPreview, sanitizeCameraForClient } = require('./demo-cameras');

function sanitizeCamera(camera) {
  if (!camera) return null;
  const { password, ...safe } = camera;
  return sanitizeCameraForClient({
    ...safe,
...
function getPreviewConfig(camera) {
  if (isDemoCamera(camera)) {
    return getDemoVideoPreview(false);
  }
  ...
  if (type === 'video-file' && /\.(mp4|webm|ogg)(\?|$)/i.test(url)) {
    return { mode: 'video', url, simulated: false, label: 'Live stream' };
  }
