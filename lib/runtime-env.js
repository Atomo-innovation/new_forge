...
function getProxyTimeoutMs(defaultMs = 4000) {
  if (process.env.MESHCENTRAL_TIMEOUT_MS) {
    return parseInt(process.env.MESHCENTRAL_TIMEOUT_MS, 10) || defaultMs;
  }
  return isServerlessRuntime() ? Math.max(defaultMs, 15000) : defaultMs;
}
