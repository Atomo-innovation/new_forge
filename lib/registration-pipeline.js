...
async function runAtomicRegistration({
  ...
  if (!skipAgentInstall && canAutoInstall(operatingSystem)) {
  ...
  } else if (meshId && skipAgentInstall) {
    const { buildAgentInstallInfo } = require('./mesh-agent-install');
    try {
      const installInfo = buildAgentInstallInfo(meshId, operatingSystem);
      meshCentral.downloadUrl = meshCentral.downloadUrl || installInfo.downloadUrl || null;
      meshCentral.installMessage = installInfo.message;
      meshCentral.platform = installInfo.platform;
    } catch {
      // ignore invalid mesh id formatting
    }
    tracker.phases.push({
      id: 'agent',
      label: 'MeshCentral agent on your device',
      status: 'done',
      message:
        'Device group created on Atomic Center. Install the MeshCentral agent on your physical device to connect it.',
    });
  }
  ...
  } else if (skipAgentInstall) {
    message =
      `Device group "${meshCentral.meshGroupName || profilePayload.meshGroupName}" is ready on Atomic Center. Install the MeshCentral agent on your physical device to connect it.`;
  ...
  return {
    ...
    success: true,
    partial: skipAgentInstall || !meshCentral.deviceOnline,
  };
}
