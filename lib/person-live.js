...
function resolveSlug(slug) {
  return slug === 'fire-smoke' ? 'fire-smoke' : 'person';
}
...
async function selectCamera(cameraId, modelSlug = 'person') {
  ...
  detectionStore.saveModelState(slug, {
    ...
    inferenceRunning: demoStream ? true : false,
...
async function startLive(cameraId, modelSlug = 'person') {
  ...
  if (isDemoMode() && isDemoCamera(camera)) {
    detectionStore.saveModelState(slug, {
      ...
      workerSource: 'demo',
      _peakToday: detectionStore.getModelState(slug)._peakToday || 0,
    });
    return {
      ok: true,
      demoMode: true,
...
async function getLiveFrame(cameraId, modelSlug = 'person') {
  ...
  if (demoStream) {
    ...
    if (state.inferenceRunning) {
      detections = generateDemoDetectionsForSlug(slug);
      const workerMeta = generateDemoWorkerMeta();
      fps = workerMeta.fps;
      inferenceMs = workerMeta.inference_ms;
    }
    ...
    if (state.inferenceRunning) {
      let newEvents = [];
      if (slug === 'person') {
        ({ newEvents } = detectionStore.recordPersonDetection(camera, detections, state, null));
      } else {
