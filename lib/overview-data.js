...
const { ensureDemoCamera } = require('./demo-cameras');
...
function getOverviewPayload(sess) {
  ...
  let cameraBlock = overview.cameras;
  let cameraFeeds = overview.cameraFeeds;

  if (isDemoMode()) {
    ensureDemoCamera();
    const stats = cameraStore.cameraStats();
    const cameras = cameraStore.listCameras();
    cameraBlock = {
      total: stats.total,
      active: stats.online,
      offline: stats.offline,
      uptimePercent: stats.total ? Math.round((stats.online / stats.total) * 100) : 100,
      offlineLastSeen: stats.offline ? 'Just now' : '—',
    };
