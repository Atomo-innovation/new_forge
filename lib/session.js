const crypto = require('crypto');
const fs = require('fs');
const path = require('path');
const { singleSessionEnabled, getSessionTtlMs, getWritableDataDir } = require('./device-config');
const { isServerlessRuntime } = require('./runtime-env');

let activeSession = null;

function getSessionFile() {
  return path.join(getWritableDataDir(), 'active-session.json');
}

function loadFromDisk() {
  try {
    const raw = fs.readFileSync(getSessionFile(), 'utf8');
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return;
    // Never persist password. Session is for UI auth + cloud sync after restarts.
    if (parsed.password) delete parsed.password;
    if (parsed.sessionId && parsed.meshUserId && parsed.username && parsed.expiresAt) {
      activeSession = parsed;
    }
  } catch {
    // ignore
  }
}

function saveToDisk() {
  try {
    const sessionFile = getSessionFile();
    fs.mkdirSync(path.dirname(sessionFile), { recursive: true });
    const safe = activeSession ? sanitizeSession(activeSession) : null;
    if (!safe) return;
    fs.writeFileSync(sessionFile, JSON.stringify(safe, null, 2));
    if (isServerlessRuntime()) {
      const { flushToBlob } = require('./vercel-persist');
      flushToBlob().catch((err) => {
        console.warn('[Session] blob flush failed:', err.message);
      });
    }
  } catch {
    // ignore
  }
}

function clearOnDisk() {
  try {
    fs.unlinkSync(getSessionFile());
  } catch {
    // ignore
  }
}

loadFromDisk();

function reloadFromDisk() {
  activeSession = null;
  loadFromDisk();
}

function restoreSessionRecord(record) {
  if (!record?.sessionId || !record?.meshUserId || !record?.username || !record?.expiresAt) {
    return false;
  }
  if (Date.now() > record.expiresAt) {
    activeSession = null;
    return false;
  }

  if (activeSession && activeSession.sessionId === record.sessionId) {
    activeSession.meshUserId = record.meshUserId || activeSession.meshUserId;
    if (record.username) activeSession.username = record.username;
    activeSession.email = record.email || activeSession.email || null;
    activeSession.clusterRoleConfirmed =
      activeSession.clusterRoleConfirmed === true || record.clusterRoleConfirmed === true;
    activeSession.userRoleConfirmed =
      activeSession.userRoleConfirmed === true || record.userRoleConfirmed === true;
    activeSession.onboardingComplete =
      activeSession.onboardingComplete === true || record.onboardingComplete === true;
    activeSession.userRole = activeSession.userRole || record.userRole || null;
    activeSession.clusterMode = activeSession.clusterMode || record.clusterMode || null;
    activeSession.expiresAt = Math.max(activeSession.expiresAt, record.expiresAt);
    return true;
  }

  if (activeSession && activeSession.sessionId !== record.sessionId) {
    const activeCreated = activeSession.createdAt || 0;
    const recordCreated = record.createdAt || 0;
    // Login/signup create a fresh in-memory session while the request still carries
    // the previous user's signed cookie — never roll back to that stale record.
    if (activeCreated >= recordCreated) {
      return true;
    }
  }

  activeSession = {
    sessionId: record.sessionId,
    meshUserId: record.meshUserId,
    username: record.username,
    email: record.email || null,
    password: null,
    clusterRoleConfirmed: record.clusterRoleConfirmed === true,
    userRoleConfirmed: record.userRoleConfirmed === true,
    userRole: record.userRole || null,
    onboardingComplete: record.onboardingComplete === true,
    clusterMode: record.clusterMode || null,
    createdAt: record.createdAt || Date.now(),
    expiresAt: record.expiresAt,
  };
  return true;
}

function patchSession(sessionId, patch) {
  const sess = getSessionRecord(sessionId);
  if (!sess || !patch || typeof patch !== 'object') return null;
  Object.assign(sess, patch);
  saveToDisk();
  return sanitizeSession(sess);
}

function createSession({ meshUserId, username, password, email }) {
  if (!singleSessionEnabled()) {
    return { sessionId: null, meshUserId, username, email: email || null };
  }

  if (
    activeSession
    && activeSession.meshUserId
    && meshUserId
    && activeSession.meshUserId !== meshUserId
  ) {
    activeSession = null;
    clearOnDisk();
  }

  const sessionId = crypto.randomBytes(32).toString('hex');
  const now = Date.now();
  activeSession = {
    sessionId,
    meshUserId,
    username,
    email: email || null,
    password: password || null,
    clusterRoleConfirmed: false,
    userRoleConfirmed: false,
    userRole: null,
    onboardingComplete: false,
    clusterMode: null,
    createdAt: now,
    expiresAt: now + getSessionTtlMs(),
  };
  saveToDisk();
  return sanitizeSession(activeSession);
}

function sanitizeSession(sess) {
  if (!sess) return null;
  const { password, ...safe } = sess;
  return { ...safe };
}

function getCurrentSessionRecord() {
  if (!singleSessionEnabled() || !activeSession) return null;
  if (Date.now() > activeSession.expiresAt) {
    activeSession = null;
    clearOnDisk();
    return null;
  }
  return activeSession;
}

function getSessionRecord(sessionId) {
  if (!singleSessionEnabled() || !activeSession) return null;
  if (sessionId && activeSession.sessionId !== sessionId) return null;
  if (Date.now() > activeSession.expiresAt) {
    activeSession = null;
    clearOnDisk();
    return null;
  }
  return activeSession;
}

function touchSessionRecord(record) {
  return restoreSessionRecord(record);
}

function getSession(sessionId) {
  return sanitizeSession(getSessionRecord(sessionId));
}

function clearSessionPassword(sessionId) {
  const sess = getSessionRecord(sessionId);
  if (!sess) return;
  delete sess.password;
}

function destroySession(sessionId) {
  if (!activeSession) return false;
  if (sessionId && activeSession.sessionId !== sessionId) return false;
  activeSession = null;
  clearOnDisk();
  return true;
}

function getActiveSession() {
  return getSession(activeSession?.sessionId);
}

function destroyAllSessions() {
  activeSession = null;
  clearOnDisk();
  return true;
}

function confirmClusterRole(sessionId) {
  const sess = getSessionRecord(sessionId);
  if (!sess) return false;
  sess.clusterRoleConfirmed = true;
  saveToDisk();
  return true;
}

function isClusterRoleConfirmed(sessionId) {
  const sess = getSessionRecord(sessionId);
  return sess ? sess.clusterRoleConfirmed === true : false;
}

function confirmUserRole(sessionId, roleId) {
  const sess = getSessionRecord(sessionId);
  if (!sess) return false;
  sess.userRoleConfirmed = true;
  sess.userRole = roleId;
  saveToDisk();
  return true;
}

function isUserRoleConfirmed(sessionId) {
  const sess = getSessionRecord(sessionId);
  return sess ? sess.userRoleConfirmed === true : false;
}

function getSessionUserRole(sessionId) {
  const sess = getSessionRecord(sessionId);
  return sess?.userRole || null;
}

module.exports = {
  createSession,
  getSession,
  getSessionRecord,
  getCurrentSessionRecord,
  clearSessionPassword,
  destroySession,
  destroyAllSessions,
  getActiveSession,
  confirmClusterRole,
  isClusterRoleConfirmed,
  confirmUserRole,
  isUserRoleConfirmed,
  getSessionUserRole,
  reloadFromDisk,
  restoreSessionRecord,
  touchSessionRecord,
  patchSession,
  sanitizeSession,
};
