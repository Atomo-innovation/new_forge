...
const DEMO_VIDEO_URL = '/demo/office.mp4';
...
function primeDemoModel(slug) {
  ...
  if (usesDemoLiveCounts(slug)) {
    ...
    const live = state._liveMetrics?.current;
    patch._liveMetrics = Number.isFinite(live) && live >= 3
      ? state._liveMetrics
      : seedDemoLiveMetrics();
  }
  detectionStore.saveModelState(slug, patch);
  const camera = cameraStore.getCamera(DEMO_CAMERA_ID);
  if (camera) detectionStore.ensureDemoEvents(slug, camera);
}
