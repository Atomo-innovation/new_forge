...
const DEMO_PREVIEW_VIDEO_URL = '/demo/wnew23.mp4';
const DEMO_DETECTION_VIDEO_URL = '/demo/dnew.mp4';
const FACE_PREVIEW_VIDEO_URL = '/demo/face1.mp4';
const FACE_DETECTION_VIDEO_URL = '/demo/face2d.mp4';
...
const DEMO_LIVE_COUNTS = [6, 7];

function getDemoLiveCount() {
  const tick = Math.floor(Date.now() / 1000);
  return tick % 2 === 0 ? DEMO_LIVE_COUNTS[0] : DEMO_LIVE_COUNTS[1];
}

function getDemoWorkerMetrics() {
  const tick = Math.floor(Date.now() / 1000);
  return {
    fps: Math.round((22.5 + (tick % 4) * 0.65) * 10) / 10,
    inference_ms: 36 + (tick % 5) * 3,
  };
}

function pickDemoLiveCount(last = null) {
  return getDemoLiveCount();
}

function seedDemoLiveMetrics() {
  const worker = getDemoWorkerMetrics();
  return {
    current: getDemoLiveCount(),
    fps: worker.fps,
    inferenceMs: worker.inference_ms,
  };
}
...
function generateDemoDetectionsForSlug(slug) {
  ...
  if (!scene || now - scene.updatedAt >= DEMO_HOLD_MS) {
    const count = getDemoLiveCount();
    const workerMeta = getDemoWorkerMetrics();
    scene = {
      count,
...
function generateDemoWorkerMeta(slug = PERSON_SLUG) {
  return getDemoWorkerMetrics();
}

function getDemoVideoPreview(inferenceRunning = false, slug = PERSON_SLUG) {
  const isFace = slug === FACE_SLUG;
  const previewUrl = isFace ? FACE_PREVIEW_VIDEO_URL : DEMO_PREVIEW_VIDEO_URL;
  const detectionUrl = isFace ? FACE_DETECTION_VIDEO_URL : DEMO_DETECTION_VIDEO_URL;
  return {
    mode: 'video',
    url: inferenceRunning ? detectionUrl : previewUrl,
    simulated: false,
...
module.exports = {
  ...
  FACE_PREVIEW_VIDEO_URL,
  FACE_DETECTION_VIDEO_URL,
  getDemoVideoPreview,
  getDemoLiveCount,
  getDemoWorkerMetrics,
  buildDemoCamera,
  ensureDemoCamera,
  isDemoCamera,
