...
function setCameras(cameras) {
  writeStore(Array.isArray(cameras) ? cameras : []);
  return readStore();
}

function upsertCamera(payload) {
  if (!payload?.id) return addCamera(payload);
  const cameras = readStore();
  const idx = cameras.findIndex((c) => c.id === payload.id);
  const camera = {
    status: 'online',
    createdAt: new Date().toISOString(),
    ...payload,
