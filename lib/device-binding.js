...
const meshPass = require('./mesh-pass');
const {
...
function getDb() {
  ...
  `);
  ensurePendingPasswordSyncTable(db);
  return db;
}

// The queue stores the new password to be pushed to Atomic Center. An earlier
// revision required an old password ('change' mode); the local forgot-password
// flow no longer collects it, so the schema is migrated to the 'forceset' shape.
function ensurePendingPasswordSyncTable(database) {
  const cols = database.prepare("PRAGMA table_info(pending_password_sync)").all();
  const hasModeColumn = cols.some((c) => c.name === 'mode');
  if (cols.length && !hasModeColumn) {
    database.exec('DROP TABLE pending_password_sync');
  }
  database.exec(`
    CREATE TABLE IF NOT EXISTS pending_password_sync (
      ...
      mesh_user_id TEXT,
      mode TEXT NOT NULL DEFAULT 'forceset',
...
async function bindUser({ meshUserId, username, email, password }) {
  ...
  if (require('./runtime-env').isServerlessRuntime()) {
    require('./vercel-persist').flushToBlob().catch(() => {});
  }
...
/**
 * Overwrite the bound user's local password WITHOUT verifying the old one.
 * Used by the offline forgot-password flow, where the user does not know (or
 * cannot supply) their current password.
 */
async function setBoundPassword({ username, newPassword }) {
  if (!offlineLoginEnabled()) {
    return { ok: false, reason: 'disabled' };
  }
  ...
  const row = getDb().prepare('SELECT username, mesh_user_id, email FROM device_binding WHERE id = 1').get();
  if (!row) {
    return { ok: false, reason: 'not_bound' };
  }
...
/**
 * Record that the bound user's password was changed locally and still needs to
 * be force-set on Atomic Center (no old password is required for that). Any
 * previously queued change is superseded by the latest new password.
 */
function queuePendingPasswordSync({ username, meshUserId, newPassword }) {
  const name = String(username || '').trim();
  ...
  getDb()
    .prepare(
      `INSERT INTO pending_password_sync
         (id, username, mesh_user_id, mode, new_password_enc, attempts, last_error, created_at, updated_at)
       VALUES (1, ?, ?, 'forceset', ?, 0, NULL, ?, ?)
       ON CONFLICT(id) DO UPDATE SET
         username = excluded.username,
         mesh_user_id = excluded.mesh_user_id,
    ...
    .run(name, meshUserId || null, newEnc, now, now);
}
